## Maintenance 2026-09-14 — bounded full-history leaderboard

The prepared leaderboard uses one control row and an indexed top-ten query. Resolution insertion, its counted receipt and the volunteer total commit in the same Convex mutation. Existing source fields and rows remain intact. A resumable operator backfill counts at most 25 resolution rows / 250,000 source bytes per page; receipts prevent repeat counts after interruption or restart. Equal scores use the earliest source creation time, then resolver ID. There are no recurring scans or new dependencies.

Activation is disabled by default. Before enabling, compare every source resolution and every derived volunteer total using the bounded internal verification pages; ensure the control total stays unchanged throughout verification, then call `leaderboardMaintenance:setEnabled` with that expected total. The count check rejects concurrent changes. `startBackfill` disables the derived reader and restarts pagination while preserving all counters and receipts; use it to reconcile uncounted records after a code rollback. Resolution source rows are append-only in current application code. Arbitrary direct database edits require reconciliation before trusting a derived total.

Local registered-handler tests cover 1,200 retained records, interrupted mutations, live resolutions interleaved with backfill, rollback recovery, unchanged source fields, activation gates and at most eleven aggregate rows per enabled public query. Full suite, build, lint, type checking and environment/promo/codegen boundary checks pass; hosted validation and production activation remain pending. The current non-expiring main backend remains energetic-bird-10. Its bounded read-only baseline has zero resolutions; historical records from the unavailable former backend are still unrecovered. This does not complete the Supabase migration or establish monthly quota headroom. Full-history ticket/TTR/SBL/location metrics are the next separate change; ingestion, queue, moderation, authentication predicates and emergency thresholds are unchanged here.
